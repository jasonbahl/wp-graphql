							...
							'node'   => [
								...
								'resolve'     => function( $source, $args, $context, ResolveInfo $info ) {
									if ( ! empty( $this->resolve_node ) && is_callable( $this->resolve_node ) ) {
										...
										return ! empty( $source['node'] ) ? $resolve_node( $source['node'], $args, $context, $info ) : [];
									} else {
										return isset( $source['node'] ) ? $source['node'] : [];
									}
								},
							],
						],
				...
				'fields'            => array_merge(
					[
						...
						'edges'    => [
							'type'        => [
								'non_null' => [
									'list_of' => $this->connection_name . 'Edge',
								],
							],
							// Translators: Placeholder is the name of the connection
