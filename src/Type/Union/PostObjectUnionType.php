 ...
 * post_type the related post can be, so in many cases it's unknow what "post_type" the related post is until the post
	...
	public function __construct() {
		...
		$config = [
			...
			'resolveType' => function( $value ) {
				return Types::post_object( $value->post_type );
			},
		];
	...
	public function getPossibleTypes() {
		...
		if ( ! empty( $allowed_post_types ) && is_array( $allowed_post_types ) ) {
			foreach ( $allowed_post_types as $allowed_post_type ) {
				if ( null === self::$possible_types[ $allowed_post_type ] ) {
