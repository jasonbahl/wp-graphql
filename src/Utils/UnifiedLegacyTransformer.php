...
class UnifiedLegacyTransformer {
	...
	public static function transform_request( string $query, array $variables = [] ): array {
		try {
			...
			foreach ( $transformation_rules as $rule_key => $rule ) {
				list( $type_name, $field_name ) = explode( '.', $rule_key );
			...
			Visitor::visit( $ast, [
				'Field' => [
					'enter' => function ( FieldNode $node ) use ( $transformation_rules, $variables, &$transformations, &$new_variables ) {
						...
						foreach ( $transformation_rules as $rule_key => $rule ) {
							if ( strpos( $rule_key, '.' . $field_name ) !== false ) {
								...
								if ( $has_legacy_args ) {
									...
									ksort( $arg_signature );
									$node_key = $field_name . ':' . json_encode( $arg_signature );
			...
			$transformed_ast = Visitor::visit( $ast, [
				'Field' => [
					'leave' => function ( FieldNode $node ) use ( $transformations, &$new_variables ) {
			...
			error_log( 'Legacy transformation failed: ' . $e->getMessage() );
			return [ 'query' => $query, 'variables' => $variables ];
		}
	}
	 ...
	 *
	 * @param FieldNode $node The field node to transform
	 ...
	 * @return FieldNode The transformed field node
	 */
	...
	/**
	 * Create a GraphQL value node from a PHP value
	 *
	 * @param mixed $value The PHP value
	 * @return mixed The GraphQL value node
	 */
	private static function create_value_node( $value ) {
		if ( is_array( $value ) ) {
			// Create object value node
			$fields = [];
			foreach ( $value as $key => $val ) {
				$fields[] = new \GraphQL\Language\AST\ObjectFieldNode( [
					'name' => new \GraphQL\Language\AST\NameNode( [ 'value' => $key ] ),
					'value' => self::create_value_node( $val ),
				] );
			}
			return new ObjectValueNode( [ 'fields' => new NodeList( $fields ) ] );
		} elseif ( is_string( $value ) ) {
			return new StringValueNode( [ 'value' => $value ] );
		}
		// Add other types as needed
		return new StringValueNode( [ 'value' => (string) $value ] );
	}
	...
	private static function generate_new_variable_declarations( $ast, array $new_variables ) {
		return Visitor::visit( $ast, [
			'OperationDefinition' => [
				'leave' => function ( $node ) use ( $new_variables ) {
