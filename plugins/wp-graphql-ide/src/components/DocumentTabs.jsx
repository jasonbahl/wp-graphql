...
export function DocumentTabs({
	...
	return (
		<div ref={containerRef} className="wpgraphql-ide-document-tabs">
			{visibleTabs.map((tab) => {
				...
				return (
					...
					>
						{editingId === tab.id ? (
							...
							<span className="wpgraphql-ide-tab-title">
								{tab.title || 'Untitled'}
							</span>
						)}
						{tabs.length > 1 && (
			...
			{overflowTabs.length > 0 && (
				<DropdownMenu
					icon={moreVertical}
					label={`${overflowTabs.length} more document${overflowTabs.length > 1 ? 's' : ''}`}
					className="wpgraphql-ide-tabs-overflow"
					toggleProps={{
						size: 'compact',
						className: 'wpgraphql-ide-tabs-overflow-btn',
					}}
				>
					{({ onClose: closeMenu }) => (
						<MenuGroup>
